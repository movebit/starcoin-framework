...
module Event {
    ...
    spec publish_generator {
        // For now this spec is not used. The module is intrinstic
        let addr = Signer::address_of(account);
        aborts_if !exists<EventHandleGenerator>(addr);
        ensures exists<EventHandleGenerator>(addr);
    }
    ...
    fun fresh_guid(generator: &mut EventHandleGenerator): vector<u8> {
        let sender_bytes = BCS::to_bytes(&generator.addr);
        let count_bytes = BCS::to_bytes(&generator.counter);
    ...
    spec fresh_guid {
        // For now this spec is not used. The module is intrinstic
        ensures generator.counter > old(generator.counter);
    }
    ...
    spec new_event_handle {
        // This is not used. The module is intrinstic
        let addr = Signer::address_of(account);
        aborts_if exists<EventHandleGenerator>(addr);
    }
    ...
    spec emit_event {
        // For now this spec is not used. The module is intrinstic
        ensures handle_ref.counter > old(handle_ref.counter);
    }
