...
module Config {
    ...
    /// Helper spec function to get Config
    spec fun spec_get<ConfigValue>(addr: address): ConfigValue {
        global<Config<ConfigValue>>(addr).payload
    }
    /// Helper spec function to get config's existence
    spec fun spec_exists<ConfigValue>(addr:address): bool {
    ...
    spec get_by_address {
        aborts_if !spec_exists<ConfigValue>(addr);
        ensures spec_exists<ConfigValue>(addr);
    ...
    spec config_exist_by_address {
        aborts_if false;
        ensures result == spec_exists<ConfigValue>(addr);
    }
    ...
    spec set {
        ...
        let post cap_opt_ = spec_cap<ConfigValue>(addr);
        let post cap_ = Option::borrow(cap_opt_);

        // The account must has be capable of modify the config
        ...
        // SPECTODO: For unknown reason we can't specify the strict abort conditions.
        ...
        ensures exists<Config<ConfigValue>>(cap_.account_address);
        ensures global<Config<ConfigValue>>(cap_.account_address).payload == payload;
    }
