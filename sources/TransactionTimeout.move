...
module TransactionTimeout {
    ...
    spec fun spec_is_valid_transaction_timestamp(txn_timestamp: u64): bool {
        if (Block::get_current_block_number() == 0) { 
            ...
            Timestamp::now_seconds() < txn_timestamp && 
                txn_timestamp < Timestamp::now_seconds() + TransactionTimeoutConfig::duration_seconds()
        }
    }
    ...
    spec is_valid_transaction_timestamp {
        include AbortsIfTimestampNotValid;
        // TODO: Functional correctness?
    }

    spec schema AbortsIfTimestampNotValid {
        let addr = CoreAddresses::GENESIS_ADDRESS();
        aborts_if !exists<Timestamp::CurrentTimeMilliseconds>(addr);
        aborts_if !exists<Block::BlockMetadata>(addr);
        include Timestamp::AbortsIfTimestampNotExists;
        aborts_if Block::get_current_block_number() != 0 && 
            Timestamp::now_seconds() + TransactionTimeoutConfig::duration_seconds() > max_u64();
        aborts_if Block::get_current_block_number() != 0 && 
            !Config::spec_exists<TransactionTimeoutConfig::TransactionTimeoutConfig>(addr);
    }
}
}
